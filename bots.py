...
class MiddlingMargaret(BaseBot):
    ...
    def _roll_bank_or_quit(self):
        ...
        if self.unbanked_points >= 900 or self.dice_remaining < 3:
            return "b"

        return "r"


# class BrendenBot(BaseBot):
#
#     def _roll_bank_or_quit(self):
#
#         if self.roller == (1, 2, 3, 4, 5, 6):
#             return "b"
#
#         return "r"


class PickyPete(BaseBot):
    """Pete only likes 1's and 5's in his rolls. Will only bank if he gets either or"""
    roll = GameLogic.roll_dice(6)

    def _roll_bank_or_quit(self):

        if any(dice in (1, 5) for dice in self.roll):
            return "b"

        return "r"


class PointMaximizingBot(BaseBot):
    """A bot that maximizes points using a rolling threshold"""

    def __init__(self, print_all=False, threshold=500):
        super().__init__(print_all=print_all)
        self.threshold = threshold

    def _roll_bank_or_quit(self):
        """decide whether to roll the dice, bank the points, or quit"""

        if self.unbanked_points >= self.threshold or self.dice_remaining < 3:
            return "b"
...
if __name__ == "__main__":
    ...
    # BrendenBot.play(num_games)
    PickyPete.play(num_games)
    PointMaximizingBot.play(num_games)
